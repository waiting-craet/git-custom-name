#!/usr/bin/env python3
"""
测试修复后的 Cloudflare Pages Functions 配置
"""
...
def test_directory_structure():
    """测试目录结构是否符合 Cloudflare Pages 要求"""
    print("测试目录结构...")
...
def test_function_file():
    ...
    try:
        ...
        # 检查是否有 Response 类
        if "class Response:" not in content:
            print("❌ 错误: 没有找到 Response 类")
            return False
        else:
            print("✅ 找到 Response 类")
...
def test_wrangler_config():
    ...
    # 检查 entry-point 是否设置为 functions
    if 'entry-point = "functions"' not in content:
        print("❌ 错误: entry-point 没有设置为 functions")
        return False
    else:
        print("✅ entry-point 正确设置为 functions")
    
    return True

def main():
    print("Cloudflare Pages Functions 配置测试")
    print("=" * 50)
